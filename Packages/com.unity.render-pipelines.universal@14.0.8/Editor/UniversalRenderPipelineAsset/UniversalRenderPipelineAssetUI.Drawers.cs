...
namespace UnityEditor.Rendering.Universal {
    ...
    internal partial class UniversalRenderPipelineAssetUI {
        ...
        static void DrawShadows(SerializedUniversalRenderPipelineAsset serialized, Editor ownerEditor) {
            ...
            // 如果Cascade数量大于4 则显示FarSplitProperty
            int cascadeCount = serialized.shadowCascadeCountProp.intValue;
            bool useFarCascade = cascadeCount > 4;
            if (cascadeCount > 4) {
                EditorGUILayout.Slider(serialized.shadowFarCascadeSplitProp, 0.0f, 1.0f, Styles.shadowFarCascadeSplitText);
                EditorGUILayout.PropertyField(serialized.shadowMapClearPerFrameProp, Styles.shadowMapClearPerFrame);
            }
            ...
            // 计算近/远Cascade的总距离 如果Count<4，则近Cascade占所有距离
            float baseMetric = serialized.shadowDistanceProp.floatValue;
            float nearMetric = useFarCascade ? baseMetric * serialized.shadowFarCascadeSplitProp.floatValue : baseMetric;
            float farMetric = baseMetric * (1.0f - serialized.shadowFarCascadeSplitProp.floatValue);
            int cascadeSplitCount = cascadeCount - 1;

            DrawCascadeSliders(serialized, cascadeSplitCount, useMetric, nearMetric);

            EditorGUI.indentLevel--;
            DrawCascades(serialized, cascadeCount, useMetric, nearMetric);
            EditorGUI.indentLevel++;

            if (useFarCascade) {
                DrawCascadeSliders(serialized, cascadeSplitCount, useMetric, farMetric, false);

                EditorGUI.indentLevel--;
                DrawCascades(serialized, cascadeCount, useMetric, farMetric, false);
                EditorGUI.indentLevel++;
            }
        ...
        // 绘制Cascade的Split滑块
        static void DrawCascadeSliders(SerializedUniversalRenderPipelineAsset serialized, int splitCount, bool useMetric, float baseMetric, bool near = true) {
            // 只在最后一个显示 last border gui
            bool showBorder = (near && splitCount <= 3) || (!near);

            float splitMax = near ? (splitCount <= 3 ? 1.0f : serialized.shadowFarCascadeSplitProp.floatValue) : 1.0f;

            if (near) splitCount = splitCount <= 3 ? splitCount : 3;
            ...
            if (near) {
                ...
                if (splitCount == 3)
                    shadowCascadeSplit = new Vector4(serialized.shadowFarCascade4SplitProp.vector3Value.x, serialized.shadowFarCascade4SplitProp.vector3Value.y, serialized.shadowFarCascade4SplitProp.vector3Value.z, 1);
                else if (splitCount == 2)
                    shadowCascadeSplit = new Vector4(serialized.shadowFarCascade3SplitProp.vector2Value.x, serialized.shadowFarCascade3SplitProp.vector2Value.y, 1, 0);
                else if (splitCount == 1)
                    shadowCascadeSplit = new Vector4(serialized.shadowFarCascade2SplitProp.floatValue, 1, 0, 0);
            }
            ...
            if (EditorGUI.EndChangeCheck()) {
                if (near) {
                    ...
                    switch (splitCount) {
                        case 3:
                            serialized.shadowFarCascade4SplitProp.vector3Value = shadowCascadeSplit;
                            break;
                        case 2:
                            serialized.shadowFarCascade3SplitProp.vector2Value = shadowCascadeSplit;
                            break;
                        case 1:
                            serialized.shadowFarCascade2SplitProp.floatValue = shadowCascadeSplit.x;
                            break;
                    }
                }
            }
        ...
        // 绘制Cascade方块
        static void DrawCascades(SerializedUniversalRenderPipelineAsset serialized, int cascadeCount, bool useMetric, float baseMetric, bool near = true) {
            // 只在最后一个需要 borderSize
            bool needBorder = (near && cascadeCount <= 4) || (!near);

            // 分成两部分
            if (near) cascadeCount = cascadeCount <= 4 ? cascadeCount : 4;
            else if (cascadeCount > 4)
                cascadeCount = cascadeCount - 4;
            else return;
            ...
            if (near) {
                ...
                if (cascadeCount == 4)
                    shadowCascadeSplit = serialized.shadowFarCascade4SplitProp.vector3Value;
                else if (cascadeCount == 3)
                    shadowCascadeSplit = serialized.shadowFarCascade3SplitProp.vector2Value;
                else if (cascadeCount == 2)
                    shadowCascadeSplit.x = serialized.shadowFarCascade2SplitProp.floatValue;
                else
                    shadowCascadeSplit.x = serialized.shadowFarCascade2SplitProp.floatValue;
            }
            ...
            cascades[lastCascade] = new ShadowCascadeGUI.Cascade() {
                ...
                borderSize = needBorder ? serialized.shadowCascadeBorderProp.floatValue : 0.0f,
            ...
            if (EditorGUI.EndChangeCheck()) {
                if (near) {
                    ...
                    if (cascadeCount == 4)
                        serialized.shadowFarCascade4SplitProp.vector3Value = new Vector3(
                            cascades[0].size,
                            cascades[0].size + cascades[1].size,
                            cascades[0].size + cascades[1].size + cascades[2].size
                        );
                    else if (cascadeCount == 3)
                        serialized.shadowFarCascade3SplitProp.vector2Value = new Vector2(
                            cascades[0].size,
                            cascades[0].size + cascades[1].size
                        );
                    else if (cascadeCount == 2)
                        serialized.shadowFarCascade2SplitProp.floatValue = cascades[0].size;
                }
